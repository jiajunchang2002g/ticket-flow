...
(function () {
  ...
  function renderGrid(seatStatuses) {
    ...
    for (var r = 0; r < selectedMovie.rows; r++) {
      ...
      for (var s = 1; s <= selectedMovie.seats_per_row; s++) {
        ...
        if (selectedSeatIDs.indexOf(seatID) !== -1) {
          btn.classList.add("seat--selected");
        }

        (function (id, unavailable) {
          btn.addEventListener("click", function () {
            if (activeSession) return;
            if (unavailable) return;
            toggleSeatSelection(id);
          });
        })(seatID, !!(info && (info.confirmed || info.booked)));
  ...
  function toggleSeatSelection(seatID) {
    var idx = selectedSeatIDs.indexOf(seatID);
    if (idx === -1) {
  ...
  function holdSelectedSeats() {
    if (activeSession || selectedSeatIDs.length === 0) return;
    api("POST", "/movies/" + selectedMovie.id + "/holds", {
      user_id: userID,
      ...
      .then(function (data) {
        activeSession = {
          ...
          movieID: data.movie_id || data.movieID || selectedMovie.id,
          seatID: data.seat_id,
          seatIDs: data.seat_ids || (data.seat_id ? [data.seat_id] : []),
  ...
  function renderCheckout() {
    ...
    if (!activeSession) {
      area.innerHTML =
        '<div class="checkout">' +
        "<h3>Selection</h3>" +
        '<div class="checkout-info"><span>Seats:</span> ' +
        (selectedSeatIDs.length
          ? escapeHtml(selectedSeatIDs.join(", "))
          : "None") +
        "</div>" +
        '<div class="checkout-info"><span>Count:</span> ' +
        selectedSeatIDs.length +
        "</div>" +
        '<div class="checkout-buttons">' +
        '<button class="btn btn--confirm" id="btnHoldSelected">Hold Selected</button>' +
        "</div>" +
        '<div id="checkoutStatus"></div>' +
        "</div>";

      var holdBtn = document.getElementById("btnHoldSelected");
      holdBtn.disabled = selectedSeatIDs.length === 0;
      holdBtn.style.opacity = selectedSeatIDs.length === 0 ? "0.5" : "1";
    ...
    area.innerHTML =
      ...
      '<div class="checkout-info"><span>Seats:</span> ' +
      escapeHtml(
        (activeSession.seatIDs || [activeSession.seatID]).join(", "),
      ) +
      "</div>" +
